...
export default function CheckoutCard() {
  ...
  useEffect(() => {
    ...
    if (paymentCancelled) {
      ...
      setPaymentMethod("");
      console.log(
        "Order details removed from localStorage due to payment cancellation"
      );
    }
  }, []);
  ...
  const handleSubmitOrder = async () => {
    ...
    if (paymentMethod === "Debit Card" && !sessionId) {
      toast.error(
        "Session ID is required for Debit Card payments. Please complete the payment process."
      );
      return;
    }
    try {
  ...
  const handlePaymentWithStripe = async () => {
    ...
    const sessionResponse = await AxiosRequest.post(
    ...
    const stripe = window.Stripe(
      "pk_test_51QY8ceGjkBvIbb1t44yllQOTlsvJ80ErOy1BXV3s4LIRVJEYnWwY10MY5Hmj8K2waRSEXkOlqwr5TSxO6fDqG5nn00PAMYNaIc"
        ...
        <Card className="w-[400px] shadow-lg shadow-black rounded-lg bg-white transform transition duration-300 hover:-translate-y-2 hover:shadow-3xl">
          ...
          <CardFooter className="flex justify-between">
            ...
            {paymentMethod === "Debit Card" ? (
              ...
              <Button disabled={!paymentMethod} onClick={handleSubmitOrder}>
                Complete Order
              </Button> // Show "Complete Order" button for other payment methods
            )}
          </CardFooter>
        </Card>
      </motion.div>
