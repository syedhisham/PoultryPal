...
const CreateFlock = () => {
  const [formData, setFormData] = useState({
    ...
    size: '',
    ageInWeeks: '',
    healthStatus: 'Healthy', // Set default value
    location: '',
    image: '',
    pricePerBird: '',
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await AxiosRequest.post('/api/flock/add', formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      toast.success('Flock and product created successfully!');
      console.log('Created Flock:', response.data);
      ...
      toast.error(err.response ? err.response.data.message : 'Failed to create flock entry');
    }
  };
...
<Typography variant="h4" className="mb-6 text-black">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <Select
              ...
              onChange={(value) => handleHealthStatusChange(value)}
              required
              color="black"
              variant="outlined"
              className="focus:!ring-0"
