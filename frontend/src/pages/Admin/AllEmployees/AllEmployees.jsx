...
if (!isAdmin) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await AxiosRequest.get('/api/employee/getAllEmployees', config);
  ...
  const handleDelete = async (userId) => {
    try {
      ...
      await AxiosRequest.delete(`/api/employee/deleteEmployee/${userId}`, config);
  ...
  const handleUpdateUserSubmit = async (e) => {
    ...
    try {
      ...
      await AxiosRequest.put(`/api/employee/updateEmployee/${selectedUserId}`, {
      ...
      setUsers(users.map(user => user._id === selectedUserId ? { ...user, firstName: newFirstName, lastName: newLastName, email: newEmail, role: newRole,payment:newPay } : user));
          ...
          {users.map(user => (
            ...
            >
              <Card className="p-6 flex flex-col gap-1 items-center text-center bg-white shadow-lg shadow-black rounded-lg transform transition duration-300 hover:-translate-y-2 hover:shadow-3xl">
                ...
                {user.payment && (
                <Typography variant="subtitle1" className="text-gray-600">Pay:{user?.payment}</Typography>
                )}
